...
class TerrariaServerPacketHandler {
  ...
  public handlePacket(server: TerrariaServer, packet: Packet): Buffer | null {
    ...
    if (packet.packetType === PacketTypes.PlayerInventorySlot) {
      // console.log(PacketTypes[packet.packetType]);
      const rd = new PacketReader(packet.data);
      const playerId = rd.readByte();
      const slotId = rd.readByte();
      const stack = rd.readInt16();
      const prefix = rd.readByte();
      const itemId = rd.readInt16();
      if (slotId === 0) {
        console.log(`${playerId} - ${itemId} (${stack}) - ${prefix}`);
      }
    }
  ...
  private handleWorldInfo(packet: Packet): boolean {
    ...
    console.log(" ");
    console.log(" ");
    console.log("SSC IS "+(this.currentServer.isSSC ? "ENABLED" : "OFF"));
