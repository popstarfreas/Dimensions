...
import { hex2str, getPacketsFromHexString, BuffersPackets, getPackedStringByteLen } from 'dimensions/utils';
...
class Client {
  ...
  public setName(name: string): void {
    ...
    if (this.globalTracking.names[name]) {
      ...
      var disconnect = new PacketWriter(4 + getPackedStringByteLen(networkText.text))
      ...
      if (this.initialConnectionAlreadyCreated) {
        ...
        this.server.socket.connect(this.server.port, this.server.ip, () => {
          ...
          if (this.options.fakeVersion.enabled) {
            ...
            let packet = new PacketWriter(3 + getPackedStringByteLen(verText))
              .setType(1)
              .packString(verText)
              .data;
  ...
  public sendChatMessage(text: string | NetworkText, color?: string | undefined): void {
    ...
    if (networkText.text.length > 0) {
      ...
      let chatMessage = new PacketWriter(10 + getPackedStringByteLen(networkText.text))
  ...
  public changeServer(server: RoutingServer, options?: ChangeServerOptions): void {
    ...
    this.server.afterClosed = () => {
      ...
      let stoned = new PacketWriter(9)
        .setType(PacketTypes.AddPlayerBuff)
        .packByte(this.player.id)
        .packByte(156)
        .packInt32(300)
        .data;
      this.socket.write(stoned);
      let webbed = new PacketWriter(9)
        .setType(PacketTypes.AddPlayerBuff)
        .packByte(this.player.id)
        .packByte(149)
        .packInt32(300)
        .data;
      this.socket.write(webbed);
      ...
      this.server.socket.connect(port, ip, () => {
        ...
        var connectPacket = new PacketWriter(3 + getPackedStringByteLen(verText))
          .setType(1)
          .packString(verText)
          .data;
