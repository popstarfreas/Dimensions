...
export class ListenServer {
  ...
  async checkBlackList(socket: Net.Socket): Promise<boolean> {
    ...
    try {
      ...
      if (blocked) {
        ...
        let text = new NetworkText("Connecting using a Host Provider is not allowed.", 0).Serialize(kickPacket);
        socket.write(new Buffer(kickPacket.data, 'hex'));
  ...
  sendCheckingIp(socket: Net.Socket): void {
    ...
    let text = new NetworkText("Checking IP...", 0).Serialize(statusPacket);

    socket.write(new Buffer(statusPacket.data, 'hex'));
  }
