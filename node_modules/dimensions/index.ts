...
class Dimensions {
  ...
  constructor(logging: Logger) {
    ...
    for (let i: number = 0; i < ConfigSettings.servers.length; i++) {
      ...
      this.listenServers[listenKey] = new ListenServer(ConfigSettings.servers[i], this.serversDetails, this.handlers, this.servers, this.options, this.globalTracking, this.logging, this.blacklist);
  ...
  reloadServers(): void {
      try {
        ...
        for (let i: number = 0; i < runAfterFinished.length; i++) {
          ...
          this.listenServers[runAfterFinished[i].key] = new ListenServer(ConfigSettings.servers[serversIndex], this.serversDetails, this.handlers, this.servers, this.options, this.globalTracking, this.logging, this.blacklist);
