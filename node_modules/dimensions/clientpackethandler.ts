...
class ClientPacketHandler {
  ...
  public handlePacket(client: Client, packet: Packet): Buffer | null {
    ...
    switch (packetType) {
      ...
      case PacketTypes.ChestItem:
        ...
        console.log(`Chest Update: ${slot} -> ${itemId} (${prefix}) [${stack}]`);
        break;

      case PacketTypes.PlaceOrKillChest:
        console.log(`Place Chest`);
        break;
    }
  ...
  private handlePlayerInventorySlot(packet: Packet): boolean {
    if ((this.currentClient.state === ClientState.FreshConnection || this.currentClient.state === ClientState.ConnectionSwitchEstablished) && !this.currentClient.waitingCharacterRestore) {
      ...
      let prefix: number = reader.readByte();
      let netID: number = reader.readInt16();
      console.log(`Inv Update: ${slotID} -> ${netID} (${prefix}) [${stack}]`);
    }
