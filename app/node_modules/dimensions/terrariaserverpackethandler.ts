...
import * as WorldInfo from "terraria-packet/src/packet/Packet_WorldInfo";
import * as PlayerInfo from "terraria-packet/src/packet/Packet_PlayerInfo.gen";
import * as NpcUpdate from "terraria-packet/src/packet/Packet_NpcUpdate.gen";
import * as ItemDropUpdate from "terraria-packet/src/packet/Packet_ItemDropUpdate.gen";
import * as PlayerSpawn from "terraria-packet/src/packet/Packet_PlayerSpawn.gen";
import * as LoadNetModule from "terraria-packet/src/packet/Packet_NetModuleLoad";
import * as Disconnect from "terraria-packet/src/packet/Packet_Disconnect";
import * as PlayerActive from "terraria-packet/src/packet/Packet_PlayerActive";
import * as PlayerInventorySlot from "terraria-packet/src/packet/Packet_PlayerInventorySlot";
...
class TerrariaServerPacketHandler {
  ...
  private handleDisconnect(packet: RawPacket): boolean {
    ...
    const disconnect = Disconnect.parse(packet.data);
  ...
  private handlePlayerActive(packet: RawPacket): boolean {
    const playerActive = PlayerActive.parse(packet.data);
  ...
  private handlePlayerInventorySlot(packet: RawPacket): boolean {
    let handled = false;
    const { playerId } = PlayerInventorySlot.parse(packet.data);
  ...
  private handleLoadNetModule(packet: RawPacket): boolean {
    const loadNetModule = LoadNetModule.parse(packet.data, true);
    switch (loadNetModule?.TAG) {
      case "TeleportPylon":
        const { pylonAction, x, y, pylonType } = loadNetModule._0;
        switch (pylonAction) {
          case "Added":
