...
import { WorldInfoPacket, PlayerInfoPacket, NpcUpdatePacket, ItemDropUpdatePacket, PlayerSpawnPacket, NetModuleLoadPacket, DisconnectPacket, PlayerActivePacket, PlayerInventorySlotPacket } from "terraria-packet";
...
class TerrariaServerPacketHandler {
  ...
  private handleDisconnect(packet: RawPacket): boolean {
    ...
    const disconnect = DisconnectPacket.parse(packet.data);
  ...
  private handleWorldInfo(packet: RawPacket): boolean {
    const worldInfo = WorldInfoPacket.parse(packet.data);
  ...
  private handleNPCUpdate(packet: RawPacket): boolean {
    const npcUpdate = NpcUpdatePacket.parse(packet.data);
  ...
  private handleUpdateItemDrop(packet: RawPacket): boolean {
    const itemDropUpdate = ItemDropUpdatePacket.parse(packet.data);
  ...
  private handlePlayerActive(packet: RawPacket): boolean {
    const playerActive = PlayerActivePacket.parse(packet.data);
  ...
  private handlePlayerInventorySlot(packet: RawPacket): boolean {
    let handled = false;
    const { playerId } = PlayerInventorySlotPacket.parse(packet.data);
