...
import * as WorldInfo from "terraria-packet/src/packet/Packet_WorldInfo.gen";
  ...
  public handlePacket(server: TerrariaServer, packet: RawPacket): Buffer | null {
    ...
    try {
      ...
      if (server.client.options.log.tServerError) {
        if (server.client.options.log.outputToConsole) {
          console.log(`Dimensions encountered an issue trying to handle a packet from the Terraria Server: ${ErrorHelper.toMessage(e)}`);
        }

        server.client.logging.appendLine(`Dimensions encountered an issue trying to handle a packet from the Terraria Server: ${ErrorHelper.toMessage(e)}`);
