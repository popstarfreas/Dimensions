...
import { WorldInfoPacket, PlayerInfoPacket, NpcUpdatePacket, ItemDropUpdatePacket, PlayerSpawnPacket, NetModuleLoadPacket, DisconnectPacket, PlayerActivePacket, PlayerInventorySlotPacket, Parser, } from "terraria-packet";
...
class TerrariaServerPacketHandler {
  ...
  public handlePacket(server: TerrariaServer, packet: RawPacket): Buffer | null {
    ...
    const parsed = Parser.parse(packet.data, true);
    if (parsed.TAG === "Error") {
      switch (parsed._0.TAG) {
        case "ReaderError":
          server.client.logging.error(`Error parsing packet: ${parsed._0._0.context}`);
          console.log(parsed._0._0.error);
          break;
        default:
          server.client.logging.error(`Error parsing packet: ${PacketTypes[packet.packetType]} ${parsed._0}`);
          break;
      }
    } else {
      console.log(parsed._0.TAG, parsed._0._0)
    }
  ...
  private handleDisconnect(packet: RawPacket): boolean {
    ...
    const disconnectResult = DisconnectPacket.parse(packet.data);
    if (disconnectResult.TAG === "Error") {
      client.logging.error(`Error parsing disconnect packet: ${disconnectResult._0}`);
      return true;
    }

    const disconnect = disconnectResult._0;

    let reason = new NetworkText(0, "Unknown Reason");
    if (disconnect !== undefined) {
      reason = new NetworkText(disconnect.reason.mode, disconnect.reason.text);
    } else {
      client.logging.error("Received a disconnect packet, but could not parse it.");
    }
  ...
  private handleWorldInfo(packet: RawPacket): boolean {
    const worldInfoResult = WorldInfoPacket.parse(packet.data);
    if (worldInfoResult.TAG === "Error") {
      return false;
    }

    const worldInfo = worldInfoResult._0;
   ...
   */
  private handleCompleteConnectionAndSpawn(_packet: RawPacket): boolean {
  ...
  private handleNPCUpdate(packet: RawPacket): boolean {
    const npcUpdateResult = NpcUpdatePacket.parse(packet.data);
    if (npcUpdateResult.TAG === "Error") {
      this.currentServer.client.logging.error(`Error parsing NPC update: ${npcUpdateResult._0}`);
      return true;
    }
    const npcUpdate = npcUpdateResult._0;
  ...
  private handleUpdateItemDrop(packet: RawPacket): boolean {
    const itemDropUpdateResult = ItemDropUpdatePacket.parse(packet.data);
    if (itemDropUpdateResult.TAG === "Error") {
      this.currentServer.client.logging.error(`Error parsing item drop update: ${itemDropUpdateResult._0}`);
      return true;
    }
    const itemDropUpdate = itemDropUpdateResult._0;
  ...
  private handlePlayerActive(packet: RawPacket): boolean {
    const playerActiveResult = PlayerActivePacket.parse(packet.data);
    if (playerActiveResult.TAG === "Error") {
      this.currentServer.client.logging.error(`Error parsing player active: ${playerActiveResult._0}`);
      return true;
    }
    const playerActive = playerActiveResult._0;
  ...
  private handlePlayerInventorySlot(packet: RawPacket): boolean {
    let handled = false;
    const inventorySlot = PlayerInventorySlotPacket.parse(packet.data)
    if (inventorySlot.TAG == "Error") {
      this.currentServer.client.logging.error(`Error parsing inventory slot: ${inventorySlot._0}`);
      return true;
    }
    const { playerId } = inventorySlot._0;
  ...
  private handlePlayerInfo(packet: RawPacket): boolean {
    const playerInfo = PlayerInfoPacket.parse(packet.data);
    if (playerInfo.TAG === "Error") {
      this.currentServer.client.logging.error(`Error parsing player info: ${playerInfo._0}`);
      return true;
    }

    const nameMismatchesRequireRewrite = this.currentServer.client.options.nameChanges?.mode === "rewrite";
    const isAboutCurrentClient = playerInfo._0.playerId === this.currentServer.client.player.id;
    const isMismatchedName = this.currentServer.client.player.name !== playerInfo._0.name;
    ...
    if (nameMismatchesRequireRewrite && isAboutCurrentClient && isMismatchedName) {
      if (!isAllowedToRename) {
        const playerInfoPacket = PlayerInfoPacket.toBuffer({ ...playerInfo._0, name: this.currentServer.client.player.name });
        if (playerInfoPacket.TAG === "Ok") {
          packet.data = playerInfoPacket._0;
        }
      } else {
        this.currentServer.client.setName(playerInfo._0.name);
      }
    }
  ...
  private handleLoadNetModule(packet: RawPacket): boolean {
    const loadNetModule = NetModuleLoadPacket.parse(packet.data, true);
    switch (loadNetModule.TAG) {
      case "Ok":
        switch (loadNetModule._0.TAG) {
          case "TeleportPylon":
            const value = loadNetModule._0;
      ...
      case "Error":
        const value = loadNetModule._0;
        this.currentServer.client.logging.error(`Error loading net module: ${value.context}`);
        break;
    }
    return false;
  }
