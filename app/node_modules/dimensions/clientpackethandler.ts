...
import { ConnectRequestPacket, PlayerInfoPacket, PlayerBuffsSetPacket, PlayerBuffAddPacket, PlayerInventorySlotPacket, PlayerManaPacket, PlayerHealthPacket, PlayerUpdatePacket, ClientUuidPacket, NetModuleLoadPacket, Parser } from "terraria-packet";
...
class ClientPacketHandler {
  ...
  public handlePacket(client: Client, rawPacket: RawPacket): Buffer | null {
    ...
    const parsed = Parser.parse(rawPacket.data, false);
    if (parsed.TAG === "Error") {
      switch (parsed._0.TAG) {
        case "ReaderError":
          client.logging.error(`Error parsing packet: ${parsed._0._0.context}`);
          console.log(parsed._0._0.error);
          break;
        default:
          client.logging.error(`Error parsing packet: ${parsed._0.TAG}`);
          break;
      }
    }
  ...
  private handleConnectRequest(packet: RawPacket): boolean {
    if (this.currentClient.version === "unknown") {
      const connectRequestResult = ConnectRequestPacket.parse(packet.data);
      if (connectRequestResult.TAG === "Error") {
        this.currentClient.logging.error(`Error parsing connect request: ${connectRequestResult._0}`);
        return true;
      }
      const connectRequest = connectRequestResult._0;
  ...
  private handlePlayerInfo(packet: RawPacket): boolean {
    ...
    if (playerInfoResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing player info: ${playerInfoResult._0}`);
      return true;
    }
  ...
  private handleUpdatePlayerBuff(packet: RawPacket): boolean {
    const playerBuffsSetResult = PlayerBuffsSetPacket.parse(packet.data);
    if (playerBuffsSetResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing player buffs set: ${playerBuffsSetResult._0}`);
      return true;
    }
    const playerBuffsSet = playerBuffsSetResult._0;
  ...
  private handleAddPlayerBuff(packet: RawPacket): boolean {
    const playerBuffAddResult = PlayerBuffAddPacket.parse(packet.data);
    if (playerBuffAddResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing player buff add: ${playerBuffAddResult._0}`);
      return true;
    }
    const playerBuffAdd = playerBuffAddResult._0;
  ...
  private handlePlayerInventorySlot(packet: RawPacket): boolean {
    if ((this.currentClient.state === ClientState.FreshConnection || this.currentClient.state === ClientState.ConnectionSwitchEstablished) && !this.currentClient.waitingCharacterRestore) {
      const playerInventorySlotResult = PlayerInventorySlotPacket.parse(packet.data);
      if (playerInventorySlotResult.TAG === "Error") {
        this.currentClient.logging.error(`Error parsing player inventory slot: ${playerInventorySlotResult._0}`);
        return true;
      }
      const playerInventorySlot = playerInventorySlotResult._0;
  ...
  private handlePlayerMana(packet: RawPacket): boolean {
    ...
    const playerManaResult = PlayerManaPacket.parse(packet.data);
    if (playerManaResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing player mana: ${playerManaResult._0}`);
      return true;
    }
    const playerMana = playerManaResult._0;
  ...
  private handlePlayerHP(packet: RawPacket): boolean {
    ...
    const playerHealthResult = PlayerHealthPacket.parse(packet.data);
    if (playerHealthResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing player health: ${playerHealthResult._0}`);
      return true;
    }
    const playerHealth = playerHealthResult._0;
  ...
  private handleUpdatePlayer(packet: RawPacket): boolean {
    const playerUpdate = PlayerUpdatePacket.parse(packet.data);
  ...
  private handleSpawnPlayer(_packet: RawPacket): boolean {
  ...
  private handleLoadNetModule(packet: RawPacket): boolean {
    const netModuleLoadResult = NetModuleLoadPacket.parse(packet.data, false);
    let handled = false;
    if (netModuleLoadResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing net module load: ${netModuleLoadResult._0}`);
      return true;
    }
    const netModuleLoad = netModuleLoadResult._0;
  ...
  private handleClientUUID(packet: RawPacket): boolean {
    const clientUuidResult = ClientUuidPacket.parse(packet.data);
    if (clientUuidResult.TAG === "Error") {
      this.currentClient.logging.error(`Error parsing client uuid: ${clientUuidResult._0}`);
      return true;
    }
