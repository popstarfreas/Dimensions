...
import { ConnectRequestPacket, PlayerInfoPacket, PlayerBuffsSetPacket, PlayerBuffAddPacket, PlayerInventorySlotPacket, PlayerManaPacket, PlayerHealthPacket, PlayerUpdatePacket, ClientUuidPacket, NetModuleLoadPacket } from "terraria-packet";
...
class ClientPacketHandler {
  ...
  private handleConnectRequest(packet: RawPacket): boolean {
    if (this.currentClient.version === "unknown") {
      const connectRequest = ConnectRequestPacket.parse(packet.data);
  ...
  private handleUpdatePlayerBuff(packet: RawPacket): boolean {
    const playerBuffsSet = PlayerBuffsSetPacket.parse(packet.data);
    ...
    if (shouldBlockInvis) {
      ...
      packet.data = PlayerBuffsSetPacket.toBuffer({ playerId: this.currentClient.player.id, buffs });
    }
  ...
  private handleAddPlayerBuff(packet: RawPacket): boolean {
    const playerBuffAdd = PlayerBuffAddPacket.parse(packet.data);
  ...
  private handlePlayerInventorySlot(packet: RawPacket): boolean {
    if ((this.currentClient.state === ClientState.FreshConnection || this.currentClient.state === ClientState.ConnectionSwitchEstablished) && !this.currentClient.waitingCharacterRestore) {
      const playerInventorySlot = PlayerInventorySlotPacket.parse(packet.data);
  ...
  private handlePlayerMana(packet: RawPacket): boolean {
    ...
    const playerMana = PlayerManaPacket.parse(packet.data);
  ...
  private handlePlayerHP(packet: RawPacket): boolean {
    ...
    const playerHealth = PlayerHealthPacket.parse(packet.data);
  ...
  private handleLoadNetModule(packet: RawPacket): boolean {
    const netModuleLoad = NetModuleLoadPacket.parse(packet.data, false);
    let handled = false;
  ...
  private handleClientUUID(packet: RawPacket): boolean {
    const clientUuid = ClientUuidPacket.parse(packet.data);
