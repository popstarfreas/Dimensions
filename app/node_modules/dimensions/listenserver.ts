...
export class ListenServer {
  ...
  public shutdown(): void {
    this.logging.info(`[${process.pid}] Server on ${this.port} is now shutting down.`);
  ...
  private handleStart(): void {
    this.logging.info(`[${process.pid}] Server on ${this.port} started.`);
  }
  ...
  private setupNewClient(clientArgs: ClientArgs, packetsAlreadyReceived: RawPacket[]): Client {
    ...
    if (this.options.log.clientConnect) {
      this.logging.info(`[${process.pid} Client: ${getProperIP(client.socket.remoteAddress)} connected [${clientArgs.server.name}: ${this.serversDetails[clientArgs.server.name].clientCount + 1}]`);
    }
