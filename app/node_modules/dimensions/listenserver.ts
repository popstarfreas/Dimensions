...
export class ListenServer {
  ...
  private async setupNewSocket(socket: Net.Socket): Promise<void> {
    ...
    if (this.options.blacklist.enabled && this.blacklist) {
      ...
      this.checkingClients.push(new BlacklistCheckClient({
  ...
  private setupNewClient(clientArgs: ClientArgs, packetsAlreadyReceived: RawPacket[]): Client {
    ...
    if (this.options.log.clientConnect) {
      if (this.options.log.outputToConsole) {
        console.log(`[${process.pid} Client: ${getProperIP(client.socket.remoteAddress)} connected [${clientArgs.server.name}: ${this.serversDetails[clientArgs.server.name].clientCount + 1}]`);
      }

      this.logging.appendLine(`[${process.pid} Client: ${getProperIP(client.socket.remoteAddress)} connected [${clientArgs.server.name}: ${this.serversDetails[clientArgs.server.name].clientCount + 1}]`);
    }
  ...
  private kickBlacklisted(client: ClientArgs): void {
    ...
    if (this.options.log.clientBlocked) {
      if (this.options.log.outputToConsole) {
        console.log(`${process.pid}] Client: ${getProperIP(client.socket.remoteAddress)} was blocked from joining.`);
      }

      this.logging.appendLine(`${process.pid}] Client: ${getProperIP(client.socket.remoteAddress)} was blocked from joining.`);
    }
  }
