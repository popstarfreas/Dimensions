...
import { getIpv4 } from 'dimensions/utils';
...
export class ListenServer {
  ...
  private decrementConnectionTracker(socket: Net.Socket): void {
    if (this.options.connectionLimit.enabled) {
      const ip = getIpv4(socket.remoteAddress as string);
  ...
  private async setupNewSocket(socket: Net.Socket): Promise<void> {
    ...
    if (this.options.log.clientConnect) {
      if (this.options.log.outputToConsole) {
        console.log(`[${process.pid} Client: ${getIpv4(socket.remoteAddress as string)} connected [${chosenServer.name}: ${this.serversDetails[chosenServer.name].clientCount + 1}]`);
      }

      this.logging.appendLine(`[${process.pid} Client: ${getIpv4(socket.remoteAddress as string)} connected [${chosenServer.name}: ${this.serversDetails[chosenServer.name].clientCount + 1}]`);
    }
    ...
    if (this.options.proxy?.enabled && this.options.proxy?.proxyType === "proxy_protocol_v1") {
      this.waitForProxyData(socket, client);
    } else {
  ...
  private async checkBlackList(socket: Net.Socket): Promise<boolean> {
    ...
    try {
      ...
      // Get blocked status
      blocked = await this.blacklist.checkIP(getIpv4(socket.remoteAddress as string) as string, this.options.blacklist.apiKey);
      if (blocked) {
        this.disconnectClient(socket, "Connecting using a Host Provider is not allowed.");

        if (this.options.log.clientBlocked) {
          if (this.options.log.outputToConsole) {
            console.log(`${process.pid}] Client: ${getIpv4(socket.remoteAddress as string)} was blocked from joining.`);
          }

          this.logging.appendLine(`${process.pid}] Client: ${getIpv4(socket.remoteAddress as string)} was blocked from joining.`);
        }
      }
    } catch (e) {
      if (this.options.log.outputToConsole) {
    ...
    socket.once('close', () => {
      ...
      try {
        ...
        if (this.options.log.clientDisconnect) {
          const logMessage = `[${process.pid}] Client: ${getIpv4(socket.remoteAddress as string)} disconnected ${client.server.name}: ${this.serversDetails[client.server.name].clientCount - 1}]`;
          if (this.options.log.outputToConsole) {
            console.log(logMessage);
          }
  ...
  private waitForProxyData(socket: Net.Socket, client: Client): void {
    let buffer = Buffer.alloc(0);
    const self = this;
    function onData(data: Buffer) {
      try {
        buffer = Buffer.concat([buffer, data]);
        const clrfIndex = buffer.toString().indexOf('\r\n');
        if (clrfIndex > -1) {
          const proxyData = buffer.slice(0, clrfIndex);
          const proxyDataParts = proxyData.toString().split(' ');
          if (proxyDataParts.length < 6) {
            self.logging.appendLine(`[${process.pid}] Proxy data received was not valid. Received: ${proxyData.toString()}`);
            client.disconnect("Unable to complete connection due to a proxy error.");
          } else {
            const [ _proxy, protocol, clientIp, _serverIp, _clientPort, _serverPort ] = proxyDataParts;
            if (protocol !== 'TCP4') {
              self.logging.appendLine(`[${process.pid}] WARNING: Proxy protocol received was not TCP4. Received: ${protocol}`);
            }
            client.ipv4 = clientIp;
            client.handleDataSend(data.slice(clrfIndex + 2));
            socket.removeListener('data', onData);
            self.hookSocketData(socket, client);
          }
        }
      } catch (e) {
        if (self.options.log.clientError) {
          if (self.options.log.outputToConsole) {
            console.log(`HandleDataSend ERROR: ${e}`);
          }

          self.logging.appendLine(`HandleDataSend ERROR: ${ErrorHelper.toMessage(e)}`);
        }
      }
    }
