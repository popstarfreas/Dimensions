...
class Dimensions {
  ...
  constructor(logging: Logger) {
    ...
    if (this.options.proxy?.enabled && (this.options.connectionLimit.enabled || this.options.connectionRateLimit.enabled)) {
      console.log("Could not start server: Connection limits and connection rate limits are not supported with proxy support enabled.");
      this.logging.appendLine("Connection limits and connection rate limits are not supported with proxy support enabled.");
      process.exit(0);
    }
    ...
    if (this.options.restApi.enabled) {
      this.restApi = new RestApi(this.options.restApi.port, this.globalTracking, this.serversDetails, this.servers, this.options.restApi.response);
    }
  }
   ...
   */
  private setUpListenServers(): void {
  ...
  /* Reloads the blacklist module from disk*/
  private reloadBlacklist(): void {
    try {
      let blackList = reload(module)('./blacklist.js').default as ({ new(): Blacklist});
      this.blacklist = new blackList();
      Object.keys(this.listenServers).forEach(key => this.listenServers[key].loadNewBlacklist(this.blacklist));
    } catch (e) {
      console.log("Error reloading Blacklist: " + e);
    }
  }
