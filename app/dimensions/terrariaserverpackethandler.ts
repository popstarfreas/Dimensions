...
class TerrariaServerPacketHandler {
  ...
  public handlePacket(server: TerrariaServer, packet: RawPacket): Buffer | null {
    ...
    try {
      ...
      if (server.client.options.log.tServerError) {
        server.client.logging.error(`Dimensions encountered an issue trying to handle a packet from the Terraria Server: ${ErrorHelper.toMessage(e)}`);
   ...
   * @return Whether or not this packet was handled (and should not be sent)
   */
  private handleWorldInfo(worldInfo: WorldInfoPacket.WorldInfo): boolean {
  ...
  private handlePlayerInventorySlot(inventorySlot: PlayerInventorySlotPacket.t, rawPacket: RawPacket): boolean {
    ...
    if (playerId === this.currentServer.client.player.id) {
      if (this.currentServer.client.state !== ClientState.FullyConnected) {
        this.currentServer.packetQueue.push(rawPacket.data);
        handled = true;
      }
    }
